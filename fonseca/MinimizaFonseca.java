...
public class MinimizaFonseca implements AGe_multifuncional.Modelo
{
    int [][][] px;
    int[][] poblacion;
    int [] m_x;
    ...
    public double[][] aptitud(int[][] poblacion, int n_funciones)
    {
        ...
        for (i=0; i < poblacion.length; i++) {
            ...
            for (k=0; k < n; k++)
                total=total + Math.pow((x[k] - (1 / Math.sqrt(n))), 2);
            for (k=0; k < n; k++)
                total2=total2 + Math.pow((x[k] + (1 / Math.sqrt(n))), 2);
            //System.out.print(" f1:"+total+ "f2:"+total2);
            aptitudes[i][0]=1 - Math.exp(-total);
            aptitudes[i][1]=1 - Math.exp(-total2);
        }
        return aptitudes;
    }
    ...
    public void finales(int [][] poblacion, int ind)
    {
        ...
        for (k=0; k < n; k++)
            total2=total2 + Math.pow((x[k] + (1 / Math.sqrt(n))), 2);
        total =1 - Math.exp(-total);
        total2=1 - Math.exp(-total2);
        System.out.println("");
