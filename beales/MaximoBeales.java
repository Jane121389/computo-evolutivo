import java.io.*;
import java.util.*;
public class MaximoBeales implements AG.Modelo
{
    int [][] px;
    int [][] py;
    int[][] poblacion;
    ...
    public void finales(int [][] poblacion, int ind)
    {
        ...
        System.out.println("valores =x:" + x + " y:" + y + " f:" + (Math.pow(1.5 - x + (x * y), 2) + Math.pow(2.25 - x + (x * Math.pow(y, 2)), 2) + Math.pow(2.625 - x + (x * Math.pow(y, 3)), 2)));
    }
