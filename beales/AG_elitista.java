...
class AG_elitista<T extends AG_elitista.Modelo>
{
    T modelo;
    int longitud;
    int individuos;
    int poblacion[][];
    double pm, pc;
    public AG_elitista(T modelo, int longitud, int individuos, double pc, double pm)
    {
        this.modelo    = modelo;
    ...
    int aplica_ag()
    {
        ...
        try
        {
            ...
            while (converge < .995)
            {
                ...
                for (i=0; i < individuos; i++)
                    if (aptitudes[i] < menor)
                    {
                        menor=aptitudes[i];
                        peor =i;
                    }
                for (i=0; i < longitud; i++)
                    nueva_poblacion[peor][i]=poblacion[mejor][i];
            ...
            System.out.print("conv:" + converge + "Generaciones:" + cont + ",");
            evaluacion=evalua();
            for (i=0; i < individuos; i++)
